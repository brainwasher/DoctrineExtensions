...
class UploadableListener extends MappedEventSubscriber {
	...
	/**
	 * Default save relatve path flag
	 *
	 * @var boolean
	 */
	private $defaultPathSaveRelative;
	...
	public function processFile(AdapterInterface $ea, $object, $action) {
		...
		// process filepath (to get rid of ..)
		$uploadSPL = new \SplFileInfo($path);

		if (!isset($config['pathSaveRelative']))
			$config['pathSaveRelative'] = $this->defaultPathSaveRelative;

		if ($action === self::ACTION_UPDATE) {
			// First we add the original file to the pendingFileRemovals array
			// FIXME: not yet working to remove original
			$removePath = $this->getFilePath($meta, $config, $object);
			// $this->pendingFileRemovals[] = $removePath;
		}
		...
		$fileSPL = new \SplFileInfo($info['filePath']);

		if ($config['pathSaveRelative'])
			$info['filePath'] = str_replace($uploadSPL->getRealPath() . DIRECTORY_SEPARATOR, '', $fileSPL->getRealPath());
	...
	public function moveFile(FileInfoInterface $fileInfo, $path, $filenameGeneratorClass = false, $overwrite = false, $appendNumber = false) {
		...
		if ($filenameGeneratorClass) {
			$filename = $filenameGeneratorClass::generate(
							str_replace($path . DIRECTORY_SEPARATOR, '', $info['fileWithoutExt']), $info['fileExtension']
			);
			$info['filePath'] = str_replace(
					DIRECTORY_SEPARATOR . $info['fileName'], DIRECTORY_SEPARATOR . $filename, $info['filePath']
			);
	...
	/**
	 * Sets the default save relative flag
	 *
	 * @param boolean $saveRelative
	 *
	 * @return void
	 */
	public function setDefaultPathSaveRelative($saveRelative) {
		$this->defaultPathSaveRelative = $saveRelative;
	}
